...
import { useServicesPage, useServices } from '@/hooks/useServices';
  ...
  return (
    <>
      ...
      <section className="relative isolate flex min-h-[60vh] items-center overflow-hidden bg-black text-white">
        <img
          src={pageData?.image || "https://images.unsplash.com/photo-1512917774080-9991f1c4c750?auto=format&fit=crop&w=1800&q=80"}
          alt={pageData?.title || ""}
          aria-hidden="true"
          ...
          <AnimatedSection animation="down">
            <div className="max-w-4xl">
              ...
              <p className="mt-8 max-w-2xl text-base leading-8 text-white/82 sm:text-lg">
                {pageData?.description || t('servicesPage.description')}
              </p>
            </div>
          </AnimatedSection>
        </div>
        ...
        <div className="container-shell grid gap-8 lg:gap-10">
          {servicesLoading ? (
            ...
            services.map((service, index) => {
              ...
              const description = service.description || t(`servicesData.${service.slug}.description`);

              return (
                <AnimatedSection key={`${service.slug}-${index}`} animation={index % 2 === 0 ? 'left' : 'right'}>
                  <article className="group h-full overflow-hidden rounded-[2rem] border border-black/5 bg-surface-strong transition duration-500 hover:-translate-y-1 hover:shadow-card dark:border-white/10">
                    <div className="grid items-center gap-0 lg:grid-cols-[1.02fr_0.98fr]">
                        ...
                        {service.show_in_projects_filter ? (
                          ...
                          >
                            <img
                              src={service.image}
                              alt={title}
                              className="aspect-[5/4] w-full object-cover transition duration-1000 group-hover:scale-[1.04] sm:aspect-[16/11] lg:aspect-[4/3]"
                            />
                          </Link>
                        ) : (
                          <div className="block">
                            <img
                              src={service.image}
                              alt={title}
                              className="aspect-[5/4] w-full object-cover sm:aspect-[16/11] lg:aspect-[4/3]"
                            />
                          </div>
                        )}
                      </div>
                      ...
                      <div className={cn('flex h-full flex-col justify-center p-6 sm:p-8 lg:p-10', index % 2 === 1 && 'lg:order-1')}>
                        ...
                        <p className="mt-5 text-base leading-8 text-ink-600 space-y-4" dangerouslySetInnerHTML={{ __html: description }}></p>
